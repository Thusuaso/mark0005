...
<script setup lang="ts">
    ...
    const mosaicFilter = await useFetch('/api/usa/filter/mosaic/' + router.currentRoute._value.params.name);
    const products = control.lang_usa_product(mosaicFilter.data._value.products,store.getLang);
    const categories = mosaicFilter.data._value.category;
    const colors = control.lang_usa_filter(mosaicFilter.data._value.color,store.getLang);
    const sizes = mosaicFilter.data._value.size;
    const usages = control.lang_usa_filter(mosaicFilter.data._value.usage,store.getLang);
    const mosaics = mosaicFilter.data._value.mosaic;
