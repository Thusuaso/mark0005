  ...
  <div class="row m-auto text-center container">
    ...
    <div class="col-sm-6 mt-2">
      ...
      <div class="row m-auto text-center" style="margin-top: 5px">
        ...
        <div
          class="col-sm-6"
          v-for="suggested in suggesteds"
          :key="suggested.id"
        >
          <ProjectsCardSuggested
            :link="suggested.link"
...
await $fetch(
  "/api/projects/detail/" + router.currentRoute._value.params.id
).then(async (res) => {
  await store.setProjectDetail(res);
});

const photos = control.lang_project_detail_photos(
  store.getProjectDetail.photos,
  store.getLang
);
let video = "";
...
console.log("store.getProjectDetail", store.getProjectDetail);
const description = control.lang_project_desc(
  store.getProjectDetail.description[0],
  store.getLang
);
const suggesteds = control.lang_project(
  store.getProjectDetail.suggested,
  store.getLang
);
const description_ = store.getProjectDescription;
const project_name = control.lang_project_detail_name(
  store.getProjectDetail.description[0],
  store.getLang
);
