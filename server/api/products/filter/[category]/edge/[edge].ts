...
export default defineEventHandler(async (event) => {
    ...
    const edge = getRouterParam(event, 'edge');
     const productsSql = `
...
TRIM('/product/detail/' + TRIM(REPLACE(LOWER(mp.urunadi_en),' ','-')) + '/' + TRIM(STR(mp.urunid))) as link
    ...
    from MekmarCom_Products mp 
        inner join MekmarCom_KenarFiltered mkf on mkf.UrunId = mp.urunid
    where mp.kategori_id='${category}' and mkf.KenarId='${edge}' and mp.yayinla=1
